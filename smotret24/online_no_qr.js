(function () {
  ...
  /*  if (!window.rch) {
      Lampa.Utils.putScript(["http://smotret24.com/invc-rch.js"], function() {}, false, function() {
        if (!window.rch.startTypeInvoke)
          window.rch.typeInvoke('http://smotret24.com', function() {});
      }, true);
    }
  */

  if (!window.rch) {
    Lampa.Utils.putScript(["http://smotret24.com/invc-rch.js"], function () {
      // Этот колбэк выполняется после успешной загрузки скрипта
      if (window.rch && typeof window.rch.typeInvoke === 'function') {
        window.rch.typeInvoke('http://smotret24.com', function () {
          console.log('typeInvoke completed');
        });
      } else {
        console.error('rch object or typeInvoke method not available');
      }
    }, false, function () {
      // Колбэк ошибки загрузки
      console.error('Failed to load invc-rch.js');
    }, true);
  } else {
    // Если rch уже существует
    if (typeof window.rch.typeInvoke === 'function') {
      window.rch.typeInvoke('http://smotret24.com', function () {
        console.log('typeInvoke completed');
      });
    }
  ...
  function rchRun(json, call) {
    if (typeof signalR == 'undefined') {
      Lampa.Utils.putScript(["http://smotret24.com/signalr-6.0.25_es5.js"], function () { }, false, function () {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
