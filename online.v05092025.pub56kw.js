(function() {
    ...
    Lampa.Utils.putScript(["https://rc.bwa.to/invc-rch.js"], function() {
      var checkRch = setInterval(function() {
        if (window.rch && window.rch[hostkey] && window.rch[hostkey].typeInvoke) {
          clearInterval(checkRch);
          window.rch[hostkey].typeInvoke('https://rc.bwa.to', function() {});
        }
      }, 50);
    }, false, function() {
  ...
  function account(url) {
    ...
    if (url.indexOf('token=') == -1) {
      var token = 'pub56kw';
      if (token != '') url = Lampa.Utils.addUrlComponent(url, 'token=pub56kw');
    }
    return url;
  }
